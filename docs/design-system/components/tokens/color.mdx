          ...
          {Object.entries(tokens.Tokens.Light.color[type][color]).map(
            ([tokenName, tokenLightInfo]) => {
              ...
              const tokenLightValue =
                tokenLightInfo.original?.value
                  ?.split('.')
                  .slice(-2)
                  .join('-')
                  .slice(0, -1) || 'N/A';
              const tokenDarkValue =
                tokenDarkInfo.original?.value
                  ?.split('.')
                  .slice(-2)
                  .join('-')
                  .slice(0, -1) || 'N/A';
  ...
  <>
    {
      <table className="docs my-6">
        ...
        <tbody>
          {Object.entries(tokens.Tokens.Light.color[type])
            .filter(
              ([tokenName]) =>
                !['brand', 'danger', 'warning', 'chart', 'utility'].includes(
                  tokenName
                )
            )
            .map(([tokenName, tokenLightInfo]) => {
              const tokenDarkInfo = tokens.Tokens.Dark.color[type][tokenName];
              const tokenLightValue =
                tokenLightInfo.original?.value
                  ?.split('.')
                  .slice(-2)
