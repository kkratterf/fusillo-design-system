...
export const IconGallery = () => {
    ...
    return (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 my-6">
            ...
            >
                <div className='flex items-center justify-center'><IconComponent className="stroke-color-icon-brand" size={32} /></div>
                <span className='w-full text-sm text-center pt-3 px-2 break-words'>{formatIconName(iconName)}</span>
            </div>
            ))}
        </div>
        {copiedIcon && (
